...
typedef struct {
    int fptr;   // filedesc
    int num_of_records_per_block;   // αριθμος εγγραφων ανα μπλοκ
    int num_of_blocks;  // αριθμος των μπλοκ
    int num_of_records; // αριθμος εγγραφων
    int num_of_buckets; // αριθμος κουβαδων
    int block_of_bucket[MAX_BUCKETS];   //  πρωτο μπλοκ του κουβα 
    int last_insert[MAX_BUCKETS];   //  τελευταιο μπλοκ του κουβα
} HT_info;

typedef struct {
    int num_of_records; // αριθμος εγγραφων στο μπλοκ
    int next_block_id;  // αναγνωριστικο του επομενου μπλοκ στην λιστα
} HT_block_info;
