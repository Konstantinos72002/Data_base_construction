...
/* Η δομή HP_info κρατάει μεταδεδομένα που σχετίζονται με το αρχείο σωρού και αποθηκεύεται στο 0 μπλοκ*/
typedef struct {
    // Να το συμπληρώσετε
    int fptr;   // αναγνωριστικός αριθμός του αρχείου
    int blk_counter; // το μπλοκ στο οποιο θα γινει το επομενο insert 
    int num_of_records_per_block;   // μέγιστες εγγραφές ανά μπλοκ
    int num_of_records; // συνολικός αριθμός εγγραφών
} HP_info;


// Αποθηκευεται στο τελος καθε μπλοκ
typedef struct{
  int next_block;   // Επομενο μπλοκ
  int num_of_records;   // Αριθμος εγγραφων στο Block
  int slot ;    // Μερος τοποθετησης εγγραφης
} HP_block_info;
